...
func host_game() -> void:
    ...
    var result: Error = enet_peer.create_server(port, max_clients)
    if result != OK:
        _set_lobby_status("Host failed: %s" % error_string(result))
        return
    ...
    _set_lobby_status("Hosting on port %d. %s" % [port, _lan_hint()])
...
func join_game(address: String) -> void:
    ...
    disconnect_game(false)
    enet_peer = ENetMultiplayerPeer.new()
    var result: Error = enet_peer.create_client(host_address, port)
    if result != OK:
        _set_lobby_status("Join failed: %s" % error_string(result))
        return
    multiplayer.multiplayer_peer = enet_peer
    online = false
    hosting = false
    connecting = true
    _set_lobby_status("Connecting to %s:%d..." % [host_address, port])
...
func disconnect_game(show_message: bool = true) -> void:
    ...
    _clear_avatars()
    if show_message:
        _set_lobby_status("Disconnected. Solo mode active.")

func toggle_lobby() -> void:
    if lobby_panel == null:
        return
    lobby_panel.visible = not lobby_panel.visible
    Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE if lobby_panel.visible else Input.MOUSE_MODE_CAPTURED)
...
func request_relay_activation(relay_id: int) -> void:
    if not online:
        ...
        _request_relay.rpc_id(1, relay_id)
...
func request_shared_shelter_action(action: String) -> void:
    ...
    if hosting:
        ...
        _request_shelter.rpc_id(1, action)
...
func request_pickup(pickup: Node) -> bool:
    ...
    _request_pickup.rpc_id(1, pickup_path)
    return false
...
func _submit_state(player_transform: Transform3D, camera_pitch: float, flashlight_on: bool, stats: Dictionary) -> void:
    ...
    if sender_id <= 1:
        return
    _apply_remote_state(sender_id, player_transform, camera_pitch, flashlight_on, stats)
    _receive_state.rpc(sender_id, player_transform, camera_pitch, flashlight_on, stats)

@rpc("authority", "call_remote", "unreliable", 0)
func _receive_state(peer_id: int, player_transform: Transform3D, camera_pitch: float, flashlight_on: bool, stats: Dictionary) -> void:
    if not online or peer_id == multiplayer.get_unique_id():
        return
    _apply_remote_state(peer_id, player_transform, camera_pitch, flashlight_on, stats)
...
func _pickup_result(pickup_path: String, accepted: bool) -> void:
    pending_pickups.erase(pickup_path)
    if not accepted:
        _objective("Another survivor took that supply first.")
        return
...
@rpc("authority", "call_remote", "reliable", 1)
func _sync_claimed(paths: Array) -> void:
...
func _send_local_state() -> void:
    ...
    if hosting:
        _receive_state.rpc(1, player.global_transform, pitch, flashlight_on, stats)
    else:
        _submit_state.rpc_id(1, player.global_transform, pitch, flashlight_on, stats)

func _apply_remote_state(peer_id: int, player_transform: Transform3D, camera_pitch: float, flashlight_on: bool, stats: Dictionary) -> void:
    ...
    _ensure_avatar(peer_id, player_transform)

func _ensure_avatar(peer_id: int, initial_transform: Transform3D) -> void:
    var existing_value: Variant = remote_avatars.get(peer_id)
    if existing_value != null:
        var existing: Node3D = existing_value as Node3D
    ...
    var parent: Node3D = scene_3d.get_node_or_null("NetworkPlayers") as Node3D
    if parent == null:
        parent = Node3D.new()
    ...
    material.albedo_color = Color.from_hsv(fmod(float(peer_id) * 0.173, 1.0), 0.45, 0.72, 1.0)
...
func _interpolate_avatars(delta: float) -> void:
    ...
    for peer_variant: Variant in remote_targets.keys():
        ...
        var avatar: Node3D = remote_avatars.get(peer_id) as Node3D
        if avatar == null or not is_instance_valid(avatar):
            continue
        var target: Dictionary = Dictionary(remote_targets.get(peer_id, {}))
        ...
        var q_current: Quaternion = current_transform.basis.get_rotation_quaternion()
        var q_target: Quaternion = target_transform.basis.get_rotation_quaternion()
        avatar.global_transform = Transform3D(
            Basis(q_current.slerp(q_target, weight)),
            current_transform.origin.lerp(target_transform.origin, weight)
        )
        ...
        if label != null:
            ...
            label.text = "SURVIVOR %d  HP %d" % [peer_id, int(round(float(stats.get("health", 100.0))))]
...
func _server_activate_relay(relay_id: int) -> void:
    ...
    if labyrinth != null and labyrinth.has_method("activate_relay"):
        labyrinth.call("activate_relay", relay_id)
    if hosting:
        _broadcast_world_state()

func _server_shelter_action(action: String) -> void:
    if not hosting:
        return
    ...
    match action:
        "generator_fuel":
            ...
            var current_fuel: float = float(shelter.get("generator_fuel_seconds"))
            shelter.set("generator_fuel_seconds", minf(max_fuel, current_fuel + per_can))
            shelter.set("generator_running", true)
        "campfire_bundle":
            var max_fire: float = float(shelter.get("campfire_max_seconds"))
            var bundle: float = float(shelter.get("campfire_bundle_seconds"))
            shelter.set("campfire_burn_seconds", minf(max_fire, float(shelter.get("campfire_burn_seconds")) + bundle))
        "campfire_wood":
            var max_fire: float = float(shelter.get("campfire_max_seconds"))
            var wood: float = float(shelter.get("campfire_wood_seconds"))
            shelter.set("campfire_burn_seconds", minf(max_fire, float(shelter.get("campfire_burn_seconds")) + wood))
        _:
            return
...
func _refresh_shelter(shelter: Node) -> void:
...
func _on_peer_connected(peer_id: int) -> void:
    if hosting:
        _sync_claimed.rpc_id(peer_id, claimed_pickups.keys())
...
func _on_connection_failed() -> void:
    ...
    connecting = false
    multiplayer.multiplayer_peer = OfflineMultiplayerPeer.new()
    _set_lobby_status("Connection failed. Check host LAN IP and port %d." % port)
...
func _on_server_disconnected() -> void:
    ...
    multiplayer.multiplayer_peer = OfflineMultiplayerPeer.new()
    _clear_avatars()
    _set_lobby_status("Host disconnected. Solo mode active.")
...
func _clear_avatars() -> void:
    for value: Variant in remote_avatars.values():
        var avatar: Node3D = value as Node3D
        if avatar != null and is_instance_valid(avatar):
            avatar.queue_free()
...
func _build_ui() -> void:
    ...
    leave_button.pressed.connect(_leave_from_ui)
    ...
    shortcut.text = "Desktop: press M to open/close CO-OP."
...
func _update_ui() -> void:
    ...
    if connecting:
        ...
        status_label.text = "CO-OP  CLIENT #%d  |  %d survivors" % [multiplayer.get_unique_id(), remote_targets.size() + 1]

func _layout_ui() -> void:
    if lobby_panel == null or coop_button == null or status_label == null:
        return
    ...
    if compact:
        ...
        coop_button.offset_top = size.y - 66.0
        coop_button.offset_right = 108.0
        coop_button.offset_bottom = size.y - 18.0
...
func _lan_hint() -> String:
    ...
    return "Share the host device LAN IPv4."
